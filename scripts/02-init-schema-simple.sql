...
CREATE POLICY IF NOT EXISTS "Published articles are viewable by everyone" ON articles
  FOR SELECT USING (status = 'published' OR (author_id = auth.uid()));

CREATE POLICY IF NOT EXISTS "Authors can create articles" ON articles
  FOR INSERT WITH CHECK (author_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Authors can update their own articles" ON articles
  FOR UPDATE USING (author_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Authors can delete their own articles" ON articles
  FOR DELETE USING (author_id = auth.uid());

-- RLS Policies for comments
CREATE POLICY IF NOT EXISTS "Approved comments are viewable by everyone" ON comments
...
CREATE POLICY IF NOT EXISTS "Article views are logged" ON article_views
  FOR INSERT WITH CHECK (true);

-- RLS Policies for subscribers
CREATE POLICY IF NOT EXISTS "Subscribers can manage subscription" ON subscribers
  FOR SELECT USING (true);

-- RLS Policies for writer invites
CREATE POLICY IF NOT EXISTS "Only admins can view invites" ON writer_invites
  FOR SELECT USING (invited_by_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Only admins can create invites" ON writer_invites
  FOR INSERT WITH CHECK (true);
