...
import {
  ...
  MessageSquare, BarChart2, UserCircle, LogOut,
} from 'lucide-react';
import { useAuth } from '@/lib/auth-context';

const menuItems = [
  { label: 'Dashboard',   href: '/writer',           icon: LayoutDashboard },
  { label: 'My Articles', href: '/writer/articles',  icon: FileText        },
  { label: 'New Article', href: '/writer/new',       icon: PenLine         },
  { label: 'Comments',    href: '/writer/comments',  icon: MessageSquare   },
  { label: 'Analytics',   href: '/writer/analytics', icon: BarChart2       },
  { label: 'My Profile',  href: '/writer/profile',   icon: UserCircle      },
];
...
export default function WriterSidebar() {
  ...
  const initials = profile?.full_name
    ...
    : 'WR';
  const displayName = profile?.full_name ?? 'Writer';
    ...
    <aside className="w-64 bg-[#0D0D0D] border-r border-white/5 h-screen sticky top-0 flex flex-col flex-shrink-0">
      ...
      <div className="px-5 py-6 border-b border-white/5 flex items-center gap-2.5">
        ...
        <div>
          ...
          <div className="text-[10px] text-neutral-500 mt-1 uppercase tracking-widest">Writer Portal</div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-3 space-y-0.5">
        {menuItems.map((item) => {
          ...
          const isActive = pathname === item.href ||
            (item.href !== '/writer' && pathname.startsWith(item.href));
          return (
      ...
      <div className="p-4 border-t border-white/5">
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <div>
            <p className="text-white text-xs font-semibold truncate max-w-[130px]">{displayName}</p>
            <p className="text-neutral-500 text-[10px]">Sports Writer</p>
          </div>
        </div>
